...
const ItemCategoryGrid = () => {
 return (
   <Box sx={{ width: { xs: "95%", sm: "90%", md: "80%" }, margin: "auto",
       padding: {xs: "10px", md: "15px" },marginBottom:"35px"
     }}>
     <Box className="max-w-[100%] mx-auto py-6 md:py-10 px-4 sm:mb-5 md:px-10">
       <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(2, 1fr)", sm: "repeat(3, 1fr)", md: "repeat(6, 1fr)" },
           gap: { xs: "15px", sm: "20px", md: "40px" },
         ...
         {itemCategory.map((item, index) => (
           ...
           >
             <Box sx={{border: "2px dotted #B0BEC5", borderRadius: "15px", width: "90%", height: "90%", display: "flex", flexDirection: "column",
                 alignItems: "center",
                 justifyContent: "center",
                 padding: "8px",
               }}>
               <Box className="bg-[#23A397] rounded-full flex items-center justify-center mb-2"
                 sx={{
                  width: { xs: "60px", sm: "70px", md: "80px" },
                  height: { xs: "60px", sm: "70px", md: "80px" },}}>
                 <Image src={item.image} alt={item.label} width={450} height={450} className="w-[80%] h-auto" />
               </Box>

               <Typography
                 variant="subtitle1"
                 className="font-bold text-[#0A2540] mb-1 text-center" sx={{ fontSize: { xs: "0.875rem", sm: "1rem" }, lineHeight: { xs: "1.2", sm: "1.4" }}}>
                 {item.label}
               </Typography>
